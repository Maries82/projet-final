...
public class Grid {
    ...
    public List<Card> giveToMarryAListOfOrderedCards(){
        List<Card> listeCards = new ArrayList<>();
        for (int j = 3; j >-1 ; j--) {
            for (int i = 0; j <4 ; j++) {
                if(isFull())
                    listeCards.add(this.grid[i][j].getCard());
    ...
    public String declareWinner(){
        ...
        for (int i = 0; i <NBR_CASES_COTE ; i++) {
            for (int j = 0; j <NBR_CASES_COTE ; j++) {
                if (null == this.grid[i][j].getCard())
                    continue;
                if (this.grid[i][j].getCard().getColor().equals("princess"))
                    count++;
            }
        }
        if (count > 8)
